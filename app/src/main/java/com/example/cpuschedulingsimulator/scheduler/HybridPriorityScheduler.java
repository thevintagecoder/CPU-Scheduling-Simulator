...
/**
 * Hybrid Priority scheduling, preemptive.
 *
 * The primary selection rule is Priority Scheduling: the
 * arrived, unfinished process with the smaller priority number
 * (higher priority) runs next.
 *
 * When two or more candidates share the same priority, Shortest
 * Job First breaks the tie: the process with the smallest
 * remaining burst time is selected.
 *
 * If priority and remaining burst time are both equal, the
 * process that arrived earlier is selected.
 */
public class HybridPriorityScheduler implements Scheduler {
     ...
     *
     * 1. Smallest priority number (Priority).
     * 2. Smallest remaining time (SJF), when priorities are equal.
