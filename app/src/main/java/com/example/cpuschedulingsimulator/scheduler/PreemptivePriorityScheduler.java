 ...
 *
 * At every time unit, the process with the highest priority
 * (smallest priority number) among the processes that have
 * already arrived is selected. A running process is preempted
 * as soon as a newly arrived process has a higher priority.
 */
public class PreemptivePriorityScheduler implements Scheduler {
    ...
    /**
     * Selects the arrived, unfinished process with the highest
     * priority (smallest priority number). Ties are broken by
